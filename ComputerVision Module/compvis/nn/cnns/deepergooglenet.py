# =============================================================================
# Implementing a Deeper GoogLeNet based on the GoogLeNet
# =============================================================================
...
class DeeperGoogLeNet:
    """GoogLeNet architecture using inception modules.
    """
    @staticmethod
    def conv_module(x, K, kX, kY, stride, chanDim, padding='same', reg=0.0005, name=None):
        ...
        # if a layer name was suppled, prepend it
        if name is not None:
            convName = name + '_conv'
            bnName = name + '_bn'
            actName = name + '_act'
    ...
    def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce, num5x5, num1x1Proj, chanDim, stage, reg=0.0005):
        # define the first branch of the Inceptuon module which consists of 1x1 convolutions
        ...
        # define the final branch of the Inception module which is the POOL projection
    ...
    def build(width, height, depth, classes, reg=0.0005):
        ...
        Args:
            width: width input
            height: height input
            classes: number of classses
            reg: regularization value by default 0.0005
        """
        # initialize the input shape to be 'channels last' and the channels dimentions itself
        ...
        # default the model input, folloed by a sequence of CONV => POOL => Pool layers
        ...
        # apply five inceptions modules followed by a POOL
        ...
        # apply a pool layer (average) followe by dropout
